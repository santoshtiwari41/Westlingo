...
const LETTER_KEYS = ["letter1", "letter2", "letter3", "letter4"];

export default function ExperienceLetterTab() {
  ...
  function handleClearFile(key: string) {
  ...
  function handleSave() {
    console.log("Saving experience letters:", selectedFiles);
    setServerFiles(selectedFiles);
    setIsEditing(false);
  }
  ...
  function UploadRow({
    ...
    handleFileChange: (key: string, file: File | undefined) => void;
    handleClearFile: (key: string) => void;
    isEditing: boolean;
    serverFiles: FileMap;
  }) {
    const file = selectedFiles[keyName];
    const serverFile = serverFiles[keyName];
    ...
    if (isEditing && isImage) {
      ...
      !isEditing &&
      serverFile &&
      serverFile.type.startsWith("image/")
    ) {
      preview = (
        <img
    ...
    return (
      <div className="flex min-h-[40px] w-full flex-col gap-2 text-xs sm:flex-row sm:items-center">
        <span className="w-32 flex-shrink-0 font-normal text-gray-600 sm:w-40">
          Letter
        </span>
          ...
          <input
            ...
            onChange={(e) => {
              if (e.target.files && e.target.files[0]) {
                handleFileChange(keyName, e.target.files[0]);
              }
            }}
          />
          <span
        ...
        {file && isEditing && (
          <span className="ml-2 flex items-center gap-1">
            ...
            <button
              ...
              onClick={() => handleClearFile(keyName)}
              title="Remove file"
            >
              <svg
                width="14"
